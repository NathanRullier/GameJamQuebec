...
func use_historien_exposition():
	...
	if historien_image == null:
		Historien_Node.get_node(conversation[dialog_index][2]).visible = true
		historien_image = conversation[dialog_index][2]
	
	if historien_image != conversation[dialog_index][2]:
		Historien_Node.get_node(historien_image).visible = false
		Historien_Node.get_node(conversation[dialog_index][2]).visible = true
		historien_image = conversation[dialog_index][2]
...
func use_hero_exposition():
	...
	if hero_image == null:
		Legend_Node.get_node(conversation[dialog_index][2]).visible = true
		hero_image = conversation[dialog_index][2]
		
	if hero_image != conversation[dialog_index][2]:
		Legend_Node.get_node(hero_image).visible = false
		Legend_Node.get_node(conversation[dialog_index][2]).visible = true
		hero_image = conversation[dialog_index][2]
